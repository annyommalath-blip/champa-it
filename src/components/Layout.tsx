...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      {/* Desktop nav */}
      <header className="hidden md:block sticky top-0 z-50 glass-header border-b border-border/30">
        <div className="max-w-7xl mx-auto flex items-center justify-between px-8 h-[56px]">
          ...
          <nav className="flex items-center gap-0.5">
            ...
            <Link to="/cart" className="relative p-2 rounded-xl hover:bg-secondary/60 transition-all active:scale-90">
              ...
              {cartCount > 0 && (
                <span className="absolute -top-0.5 -right-0.5 min-w-[17px] h-[17px] rounded-full bg-foreground text-background text-[9px] flex items-center justify-center font-bold px-1 ring-2 ring-background">
                  {cartCount}
                </span>
              )}
            </Link>
      ...
      {/* Mobile top bar */}
      <header className="md:hidden px-5 h-[52px] glass-header sticky top-0 z-50 border-b border-border/30 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2">
          <img src={logo} alt="Champa" className="h-7 w-7 rounded-[10px] object-cover shadow-sm" />
          <span className="font-extrabold text-[15px] text-foreground tracking-tight">Champa</span>
        </Link>
        <div className="flex items-center gap-0">
          <Link to="/cart" className="relative p-2.5 rounded-xl active:scale-90 transition-transform">
            <ShoppingCart className="w-[19px] h-[19px] text-foreground/60" strokeWidth={1.8} />
            {cartCount > 0 && (
              <span className="absolute top-1 right-1 min-w-[15px] h-[15px] rounded-full bg-foreground text-background text-[9px] flex items-center justify-center font-bold px-0.5">
                {cartCount}
              </span>
            )}
          </Link>
          <Link to="/notifications" className="p-2.5 rounded-xl active:scale-90 transition-transform">
            <Bell className="w-[19px] h-[19px] text-foreground/60" strokeWidth={1.8} />
          </Link>
        </div>
      </header>

      <main className="flex-1 md:pb-0 pb-[76px]">{children}</main>
          ...
          {navKeys.map((item) => {
            ...
            return (
              <Link
                key={item.to}
                to={item.to}
                className="flex flex-col items-center justify-center gap-[5px] py-1.5 min-w-[52px] transition-all active:scale-90"
              >
                <div className="relative">
                  {active && (
                    <div className="absolute -inset-2 rounded-xl bg-foreground/[0.06]" />
                  )}
                  <Icon
                    className={`w-[21px] h-[21px] relative transition-colors ${
      ...
      <footer className="hidden md:block border-t border-border/30 bg-card">
        <div className="max-w-7xl mx-auto px-8 py-12 grid grid-cols-4 gap-10">
          ...
          <div>
            <h4 className="text-micro font-bold uppercase tracking-[0.15em] text-muted-foreground/70 mb-4">{t("footer.products")}</h4>
          ...
          <div>
            <h4 className="text-micro font-bold uppercase tracking-[0.15em] text-muted-foreground/70 mb-4">{t("footer.services")}</h4>
            <div className="flex flex-col gap-2.5 text-caption">
          ...
          <div>
            <h4 className="text-micro font-bold uppercase tracking-[0.15em] text-muted-foreground/70 mb-4">{t("footer.company")}</h4>
            <div className="flex flex-col gap-2.5 text-caption">
        ...
        <div className="border-t border-border/30 px-8 py-4 text-center text-micro text-muted-foreground/60">
