...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1 md:pb-0 pb-[80px]">{children}</main>
          ...
          {navKeys.map((item) => {
            ...
            return (
              <Link
                key={item.to}
                to={item.to}
                className="flex flex-col items-center justify-center gap-1 py-1.5 min-w-[52px] transition-all active:scale-90"
              >
                <Icon
