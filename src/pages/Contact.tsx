  ...
  return (
    <div>
      ...
      <div className="hero-dark">
        <div className="max-w-4xl mx-auto px-4 py-12 md:py-16 md:px-8 text-center">
          <h1 className="text-3xl md:text-4xl font-bold mb-3">Contact Sales</h1>
          <p className="text-white/70">Get in touch with our team. We typically respond within 1 hour.</p>
        </div>
      </div>
      ...
      {!chatOpen && (
        <button
          ...
          className="fixed bottom-6 right-6 w-14 h-14 rounded-full bg-primary text-primary-foreground flex items-center justify-center shadow-lg hover:brightness-110 transition-all z-40"
...
function ChatWidget({ onClose, guestId, conversations, addConversation, addMessage, addNotification }: any) {
  ...
  return (
    <div className="fixed bottom-6 right-6 w-[calc(100vw-2rem)] max-w-sm z-50 rounded-xl border border-border bg-card overflow-hidden flex flex-col shadow-xl" style={{ height: "28rem" }}>
        ...
        {chatMessages.map((msg: any) => (
          <div key={msg.id} className={`flex ${msg.senderType === "user" ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[80%] px-3.5 py-2 rounded-xl text-sm ${
              msg.senderType === "user" ? "bg-primary text-primary-foreground rounded-br-sm" : "bg-card border border-border rounded-bl-sm"
            }`}>
              {msg.content}
            </div>
          </div>
