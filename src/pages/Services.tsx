...
import { ArrowRight, Server, Cloud, Shield, HeadphonesIcon, BarChart3, Wrench, Clock, ChevronRight, Check, Upload, ArrowLeft, Loader2 } from "lucide-react";
    ...
    if (submitted) {
      return (
        <div className="px-5 py-16 md:max-w-lg md:mx-auto text-center animate-fade-in">
          ...
          <div className="app-card p-4 mb-6 text-left">
            <h3 className="text-caption font-semibold text-muted-foreground uppercase tracking-wider mb-3">Status Tracker</h3>
            {["Submitted", "Under Review", "Quote Sent", "Scheduled"].map((step, i) => (
              <div key={step} className="flex items-center gap-3 py-2">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-micro font-bold ${
    ...
    return (
      <div className="px-5 py-4 md:max-w-lg md:mx-auto animate-fade-in">
        ...
        <button onClick={() => { if (wizardStep === 0) { setSelectedService(null); } else { setWizardStep((wizardStep - 1) as WizardStep); } }}
          className="flex items-center gap-1.5 text-caption text-muted-foreground mb-4 active:scale-95 transition-transform">
        ...
        <div className="flex items-center gap-1 mb-6 mt-3">
          {stepLabels.map((label, i) => (
            <div key={label} className="flex items-center gap-1 flex-1">
              <div className={`h-1 rounded-full flex-1 transition-colors ${i <= wizardStep ? "bg-primary" : "bg-border"}`} />
            </div>
          ))}
        </div>
        <p className="text-micro text-muted-foreground mb-4">Step {wizardStep + 1} of 5 — {stepLabels[wizardStep]}</p>
        ...
        {wizardStep === 0 && (
          <div className="space-y-3">
            {[
              ...
              <div key={f.key}>
                <label className="text-caption text-muted-foreground mb-1 block">{f.label}</label>
        ...
        {wizardStep === 2 && (
          <div className="space-y-4">
            ...
            <div>
              <label className="text-caption text-muted-foreground mb-1 block">Preferred schedule</label>
      ...
      <div className="px-5 md:px-8 pb-8">
        ...
        <div className="space-y-2">
          {services.map((s) => (
            ...
            >
              <s.icon className="w-5 h-5 text-muted-foreground shrink-0" strokeWidth={1.6} />
