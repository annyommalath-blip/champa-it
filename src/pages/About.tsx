...
import { ArrowRight, ShoppingBag, Wrench, MessageCircle, LayoutDashboard, Zap, Tag, Star, Users, Clock, TrendingUp, CheckCircle, ChevronRight, Pencil, Settings } from "lucide-react";
    ...
    <div className="px-5 py-4 space-y-7 md:max-w-7xl md:mx-auto md:px-8 md:py-8 md:space-y-12">
      ...
      {isAdmin && (
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-primary/10 border border-primary/20">
          ...
          <Link to="/admin" className="ml-auto text-xs font-semibold text-primary hover:underline">
            Open Admin Portal →
          </Link>
        </div>
      )}
            ...
            {heroSlides.map((slide, i) => (
              <div key={i} className="flex-[0_0_100%] min-w-0 px-1">
                <div className="app-card relative overflow-hidden p-6 md:p-10 min-h-[160px] md:min-h-[240px] flex flex-col justify-end">
                  ...
                  <Link to={slide.link} className="self-start inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-primary text-primary-foreground text-sm font-semibold transition-all hover:brightness-110">
      ...
      <section>
        ...
        <div className="grid grid-cols-3 gap-3">
          {quickActions.map((action) => (
            <Link key={action.label} to={action.link} className="app-card flex flex-col items-center gap-2 py-4 px-2 active:scale-95 transition-transform">
      ...
      <section>
        ...
        <div className="flex gap-3 overflow-x-auto snap-x snap-mandatory pb-2 -mx-5 px-5 scrollbar-hide">
          {featuredProducts.map((product) => (
            <Link key={product.id} to={`/shop/${product.id}`} className="app-card flex-shrink-0 w-40 md:w-48 snap-start overflow-hidden relative group">
              {/* Admin edit button */}
              {isAdmin && (
                <button
                  onClick={(e) => openEdit(product, e)}
                  className="absolute top-2 right-2 z-10 w-7 h-7 rounded-lg bg-background/90 border border-border flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-primary/10 hover:border-primary/30"
                  title="Edit"
                >
              ...
              <div className="p-3">
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  <div className="flex items-center gap-0.5">
                    ...
                    <span className="text-[11px] text-muted-foreground">{product.rating ?? 0}</span>
                  </div>
                </div>
              </div>
            </Link>
      ...
      <section className="pb-4">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 -mx-5 px-5 scrollbar-hide">
          {partners.map((name) => (
            <span key={name} className="flex-shrink-0 px-4 py-2 rounded-xl bg-secondary/50 border border-border/50 text-xs font-semibold text-muted-foreground whitespace-nowrap">
              {name}
            </span>
          ))}
        </div>
